...
// Use WSL2 IP address for Windows Claude Desktop connectivity
const SERVER_HOSTNAME = '172.20.1.55';
const SERVER_PORT = 6174;
...
rl.on('line', (line) => {
  ...
  try {
    ...
    if (message.method === 'initialize') {
      ...
      const req = http.request(options, (res) => {
        ...
        res.on('end', () => {
          ...
          process.stderr.write(`[Claude Desktop Bridge] Server initialized successfully\n`);
        });
      });
      req.on('error', (error) => {
        process.stderr.write(`[Claude Desktop Bridge] Init error: ${error.message}\n`);
      });
      req.write(postData);
      req.end();
      return;
    ...
    req.on('error', (error) => {
      process.stderr.write(`[Claude Desktop Bridge] Request error: ${error.message}\n`);
      if (originalId !== undefined) {
        const errorResponse = {
          jsonrpc: '2.0',
          id: originalId,
    ...
    process.stderr.write(`[Claude Desktop Bridge] Parse error: ${error.message}\n`);
  }
});

// Handle process termination
process.on('SIGINT', () => {
  process.stderr.write('[Claude Desktop Bridge] Shutting down...\n');
  process.exit(0);
});

process.on('SIGTERM', () => {
  process.stderr.write('[Claude Desktop Bridge] Terminated\n');
  process.exit(0);
});

// Log ready to stderr
process.stderr.write(`[Claude Desktop Bridge] Ready - connecting to ${SERVER_HOSTNAME}:${SERVER_PORT}\n`);