...
- Evidence attribution: reviewer PASS and approval observations must be written through `add_observations` with top-level `agentId` set to the intended row creator. The current `agentId` override is an operator/API-key trust-boundary assertion, not cryptographic identity proof. The gate validates `shared_memory.created_by` plus operator-pinned observation IDs, exact dry-run hash, exact scope, and evidence metadata, so hand-editing `created_by` is not an approved production evidence path.
...
- Production rollback is intentionally manual: use the owner audit id from execute, snapshot the watched table counts, then in a reviewed SQL session deactivate `entity_identities`, `entity_context_facets`, and `entity_lookup_identity_links` rows for that `source_audit_id`; if identities from the batch are deactivated, also review any active facets/links attached to those identities because rollback is identity-scoped and may cascade beyond the exact audit id. Record a `pass2_rollback` audit row and rerun `verify`/count checks after the SQL change.
